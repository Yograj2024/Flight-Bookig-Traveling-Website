...
const LinksSection = ({ title, links, customStyle }) => {
  return (
    <div className={`m-[20px_0px] md:flex-[1%] ${customStyle}`}>
      <h5 className={`text-[1.8rem] text-white font-semibold sm:text-left mb-[20px] mt-[40px] lg:mt-[0px]`}>
        {title}
      </h5>
        ...
        {links.map((item, index) => {
          ...
          return (
            <h5 className={`m-[5px_0px] text-slate-300`} key={`${index}_${item}`}>
              ...
              <Link to={`/${formattedUrl == "home" ? "" : formattedUrl}`}>{item}</Link>
            </h5>
          );
        })}
      </div>
      ...
      <section className="">
        <div className={`p-[0px_20px_0px] md:pt-[70px] md:flex md:flex-col lg:flex-row md:max-w-[1440px] md:m-auto`}>
          <div className={`m-[50px_0] md:m-0 md:p-[30px_0_0] md:w-ful lg:w-[380px] border-t-2 border-t-slate-700 p-[50px_0px_0px] md:border-none`}>
            <div className={`h-[90px] w-[300px]  m-[0px_0px_30px]`}>
          ...
          <div className='sm:grid sm:grid-cols-2 md:grid-cols-3 md:mt-[30px] lg:mt-0'>
            <LinksSection title="Quick Links" links={qLinks} customStyle={"lg:pl-[70px]"} />
            <LinksSection title="Useful Links" links={usefulLinks} customStyle={"lg:pl-[60px]"} />

            <div className={`m-[20px_0px] md:flex-[6%]`}>
              <h5 className={`text-[1.8rem] text-white font-semibold sm:text-left mb-[20px] mt-[40px] lg:mt-0 md:pl-[70px]`}>Contact Info</h5>
              {
