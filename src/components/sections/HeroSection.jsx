...
const HeroSection = () => {
    ...
    return <section className  = {`relative h-[650px] sm:h-[561px] flex m-[0px_0px_0px] before:bg-blue-100 before:content-[''] flex-col md:w-full before:h-full before:w-full before:bg-[url('https://images.unsplash.com/photo-1710330759524-c89e93b4217f?fm=jpg&q=60&w=3000&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')] before:bg-no-repeat before:bg-cover before:bg-[50%_0%] sm:before:bg-[0%_0%] before:absolute before:z-[-1] bg-black bg-opacity-40`} >
        <div className={`w-[90%] lg:max-w-[1440px] m-[30px_auto_0] `}>
            <h1 className= {`text-center text-[1.5rem] md:text-[2.5rem] text-transparent bg-clip-text  bg-gradient-to-r from-red-500  via-orange-500 to-pink-500  font-bold tracking-[0.05rem] `} style={{trasform:'rotate(-10deg)',transformOrigin:'center'}}> Bigen your adventure here  </h1>
            <BookingForm/>
        </div>
    </section>
};
                    ...
                    <form action="" className  ={`flex flex-col relative text-[0.9rem] w-full gap-y-[30px] pt-[30px]`}>
                        <div className ="flex flex-col gap-y-[8px] md:flex-row md:items-center md:w-[90%] md:m-auto md:gap-x-[5px]">
                            <div className = {`flex flex-col w-[90%] m-auto gap-[5px] text-white`}>
                                ...
                                <input type="text" id="origin" value={originVal} className={`relative h-[50px] W-[20px] rounded-[8px] outline-none p-2 bg-transparent border`} onClick = { () =>{
                                    setAmIvisibleOrigin(!amIvisibleOrigin);
                                    amIvisibleDeparture ? setAmIvisibleDeparture(!amIvisibleDeparture) : ""
                                }} />
                                    <SuggestionBox position={'left-[7rem]'} amIvisible={amIvisibleOrigin} setAmIvisible={setAmIvisibleOrigin} inputVal={originVal} setInputvar={setOriginVar}  />
                            </div>
                            ...
                            <div className = {`flex flex-col w-[90%] m-auto gap-[5px] text-white`}>
                                ...
                                <input type="text" value={departureVal} id="destination" className  ={`relative h-[50px] W-[20px] rounded-[8px] outline-none p-2 bg-transparent border `} onClick={ () => {
                                    setAmIvisibleDeparture(!amIvisibleDeparture);
                                    amIvisibleOrigin ? setAmIvisibleOrigin(!amIvisibleOrigin) : ""
                                }
                                } />
                                <SuggestionBox position={'right-[5rem]'} amIvisible={amIvisibleDeparture} setAmIvisible={setAmIvisibleDeparture} setInputvar={setDepartureval}  />
                            </div>
                        </div>
                        ...
                        <div className ={`flex w-[90%] m-auto gap-[10px]`} >
                            <div className ={` flex flex-col w-[90%] m-auto gap-[5px] text-white`}>
                                ...
                                <input type="date" id='depart-date' onClick={ () => {}} className= {` h-[40px] w-[100%] p-2 rounded-[8px] outline-none bg-transparent border`} />
                            </div>
                            <div className ={`flex flex-col w-[90%] m-auto gap-[5px] text-white`}>
                                <label htmlFor="return">Return Date</label>
                                <input type="date" id='return-date' onClick={ () => {}} className ={`relative h-[40px] w-[100%] p-2 rounded-[8px] outline-none bg-transparent border`} />
                            </div>
                        </div>
